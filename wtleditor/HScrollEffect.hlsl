...
VSOut GeometryVS(VSIn input)
{
    ...
    float X = 0.5f;
	float radius = X * size.x / 2.0f / 3.14159f;
	float angle = (input.meshPosition.x - xRatio) * size.x / radius; 
	float bangle = (input.meshPosition.x - xRatio) * X;

	if (input.meshPosition.x > xRatio)
	{
		output.sceneSpaceOutput.x = size.x * xRatio + radius * (1.0f - bangle) * sin(angle) ;
	}
	else 
	{
		output.sceneSpaceOutput.x = size.x * input.meshPosition.x;
    ...
    if (input.meshPosition.x > xRatio)
	{
		output.sceneSpaceOutput.z = size.x / 2.0f * (1.0f - bangle) * cos(angle);
	}
	else 
	{
		output.sceneSpaceOutput.z = size.x / 2.0f; 
	}
	...
	output.clipSpaceOutput.z = 1 - (size.x / 2.0f - output.sceneSpaceOutput.z) / size.x / 2.0f;
