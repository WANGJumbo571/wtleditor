...
HRESULT __stdcall VScrollEffect::CreateWaveImpl(_Outptr_ IUnknown** ppEffectImpl)
{
	*ppEffectImpl = static_cast<ID2D1EffectImpl*>(new (std::nothrow) VScrollEffect());
...
HRESULT VScrollEffect::Register(_In_ ID2D1Factory1* pFactory)
{
	PCWSTR propertyXml =
		XML(
			<?xml version = '1.0'?>
			<Effect>
			<!--System Properties-->
			<Property name = 'DisplayName' type = 'string' value = 'Wave 2 Effect'/>
			<Property name = 'Author' type = 'string' value = 'Microsoft Corporation'/>
			<Property name = 'Category' type = 'string' value = 'Sample'/>
			<Property name = 'Description' type = 'string' value = 'Animation of a v wave.'/>
			<Inputs>
			<Input name = 'Source'/>
			</Inputs>
			<!--Effect - specific Properties-->
			<Property name = 'SizeX' type = 'float' value = '0'>
			<Property name = 'DisplayName' type = 'string' value = 'Size x'/>
			<Property name = 'Min' type = 'float' value = '-2000.0'/>
			<Property name = 'Max' type = 'float' value = '2000.0'/>
			<Property name = 'Default' type = 'float' value = '0.0'/>
			</Property>
			<Property name = 'SizeY' type = 'float' value = '0'>
			<Property name = 'DisplayName' type = 'string' value = 'Size y'/>
			<Property name = 'Min' type = 'float' value = '-2000.0'/>
			<Property name = 'Max' type = 'float' value = '2000.0'/>
			<Property name = 'Default' type = 'float' value = '0.0'/>
			</Property>
	...
	{
		D2D1_VALUE_TYPE_BINDING(L"SizeX",  &SetSizeX,   &GetSizeX),
		D2D1_VALUE_TYPE_BINDING(L"SizeY",  &SetSizeY,   &GetSizeY),
...
HRESULT VScrollEffect::Initialize(
	_In_ ID2D1EffectContext* pEffectContext,
	_In_ ID2D1TransformGraph* pTransformGraph
)
{
	LONG len;
	byte* data = FReadData("VScrollEffect.cso", &len);
	if (data == nullptr)
	{
		return S_FALSE;
	}

	HRESULT hr = pEffectContext->LoadVertexShader(GUID_WaveTransformVertexShaderV, data, len);

	if (SUCCEEDED(hr))
	{
		// Only generate the vertex buffer if it has not already been initialized.
		pEffectContext->FindVertexBuffer(&GUID_WaveTransformVertexBufferV, &m_vertexBuffer);
		if (m_vertexBuffer == nullptr)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				hr = pEffectContext->CreateVertexBuffer(
					&vbProp,
					&GUID_WaveTransformVertexBufferV,
					&cvbProp,
					&m_vertexBuffer
				);
			}
...
HRESULT VScrollEffect::GenerateMesh(_Outptr_ Vertex** ppMesh)
{
...
IFACEMETHODIMP VScrollEffect::PrepareForRender(D2D1_CHANGE_TYPE changeType)
{
...
HRESULT VScrollEffect::SetSkewX(float skew)
{
	m_skewX = Clamp(skew, -5.0f, 5.0f);
	return S_OK;
}

HRESULT VScrollEffect::SetSkewY(float skew)
{
	m_skewY = Clamp(skew, -5.0f, 5.0f);
	return S_OK;
}

HRESULT VScrollEffect::SetXRatio(float ratio)
{
...
HRESULT VScrollEffect::SetWaveOffset(float waveOffset)
{
...
// The wave offset is not clamped since no min or max is specified.
HRESULT VScrollEffect::SetSizeX(float sizeX)
{
	m_sizeX = sizeX;
	return S_OK;
}

float VScrollEffect::GetSizeX() const
{
	return m_sizeX;
}

float VScrollEffect::GetSizeY() const
{
	return m_sizeY;
}

HRESULT VScrollEffect::SetSizeY(float sizey)
{
	m_sizeY = sizey;
	return S_OK;
}
...
float VScrollEffect::GetSkewX() const
{
	return m_skewX;
}

float VScrollEffect::GetSkewY() const
{
	return m_skewY;
}

float VScrollEffect::GetWaveOffset() const
{
	return m_waveOffset;
}

float VScrollEffect::GetXRatio() const
{
	return m_xRatio;
}

HRESULT VScrollEffect::UpdateConstants()
{
...
IFACEMETHODIMP VScrollEffect::MapOutputRectToInputRects(
...
IFACEMETHODIMP VScrollEffect::MapInputRectsToOutputRect(
...
IFACEMETHODIMP VScrollEffect::MapInvalidRect(
	UINT32 inputIndex,
...
IFACEMETHODIMP VScrollEffect::SetDrawInfo(
...
)
{
	...
	return m_drawInfo->SetVertexProcessing(
		...
		&range,
		&GUID_WaveTransformVertexShaderV
	);
}
...
IFACEMETHODIMP_(ULONG) VScrollEffect::AddRef()
{
	m_refCount++;
	return m_refCount;
}

IFACEMETHODIMP_(ULONG) VScrollEffect::Release()
{
	m_refCount--;
...
IFACEMETHODIMP VScrollEffect::QueryInterface(
	_In_ REFIID riid,
